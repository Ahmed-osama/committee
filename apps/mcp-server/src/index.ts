  ...
  'committee_plan',
  {
    ...
    inputSchema: {
        ...
        .array(
          z.object({
            role: z
              ...
              .describe(
                'Behavioral seat: skeptic gates finalization on real objections; skeptic/devils_advocate speak as challengers; the rest propose.',
              ),
            name: z
              .string()
              .describe(
                'Creative title for this seat, tailored to the task, e.g. "Migration Skeptic".',
              ),
            persona: z
              .string()
              .describe(
                '1-3 sentence system prompt for this seat: its specific expertise and what it should focus on for this goal.',
              ),
          }),
        )
        .min(2)
      ...
      maxTurns: z
        ...
        .max(100)
        .optional()
        .describe('Safety ceiling on conversation length (default 36).'),
      visual: z
        .boolean()
        .optional()
        .describe(
          'Also generate an SVG diagram of the finalized plan once it lands (default true).',
        ),
    },
  },
    ...
    if (roster.length === 0) {
      return {
        ...
        content: [
          {
            type: 'text' as const,
            text: 'No committee agent has a usable provider right now — check API keys / rate limits.',
          },
        ],
      };
    }
    ...
    if (!result.finalized || !result.plan) {
      ...
      return {
        ...
        content: [
          {
            type: 'text' as const,
            text: `No plan reached within ${result.turnsUsed} turns. Conversation id: ${conversation.id}`,
          },
        ],
      };
    }
...
server.registerTool(
  'committee_resolve_linear_target',
  {
    ...
    inputSchema: {
      summary: z
        .string()
        .describe('A short summary or title of the spec/goal to resolve a target for.'),
    },
  },
  async ({ summary }) => {
