...
- `pnpm run docs:dev` — serve the curriculum docs site (`apps/docs`, Astro) at
  http://localhost:4321. `pnpm run docs:build` statically generates it to `apps/docs/dist`.
- `pnpm run format` / `pnpm run format:check` — Prettier, repo-wide. See
  `docs/CODING_STYLE.md`'s Formatting section.
...
- Whenever a `committee_plan` session finalizes a new mini-startup/initiative (one getting
  its own Linear project), add it to "Mini-startups built here" below _and_ create its
...
- `apps/docs/src/content/` — the curriculum docs site's actual content (markdown, one
  topic per file, ordered per repo section). `document-update` and `learning-digest`
  keep it current; see `apps/docs/CLAUDE.md`.
...
_converse_ to plan, with Linear as the task backend, not a rebuilt project-management
